...
3. **Upload to PyPI**
   ```bash
   .venv/Scripts/python -m twine upload --disable-progress-bar dist/mapnetwork_mcp-<version>*
   ```
   Requires a PyPI API token (via `~/.pypirc` or `TWINE_USERNAME=__token__` / `TWINE_PASSWORD=...`).

   ⚠️ On Windows, omitting `--disable-progress-bar` can crash twine with
   `UnicodeEncodeError: 'cp932' codec can't encode character '•'`
   (the legacy console can't render `rich`'s progress-bar glyphs). The crash
   happens in the display code, not the upload itself, so **check whether the
   upload actually went through before retrying** — re-uploading a version
   that already landed is rejected by PyPI:
   ```bash
